...
func main() {
	...
	// initialize request
	// Safely extract the 4th segment of the license key (index 3). If the license key
	// isn't in the expected format, fall back to a placeholder to avoid panics.
	licenseSegment := "unknown"
	if global.Config.License.Key != "" {
		parts := strings.Split(global.Config.License.Key, "-")
		if len(parts) > 3 {
			licenseSegment = parts[3]
		}
	}

	var userAgent string = fmt.Sprintf("Discord/%d-IS/%s-%d", global.DiscordBuildNumber, licenseSegment, time.Now().UnixNano())
