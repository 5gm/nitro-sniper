...
func GetDiscordBuildNumber() (int, error) {
	makeGetReq := func(urlStr string) ([]byte, error) {
		...
		// send a browser-like user agent and common headers so Discord returns the expected HTML
	...
	// More permissive regex: allow letters, digits, dot, underscore and hyphen; case-insensitive
	discordFiles := regexp.MustCompile(`(?i)assets/[a-z0-9._-]+\.js`).FindAllString(string(responeBody), -1)

	// If we didn't find matches, try to find full URLs that include assets (CDN fallback)
	if len(discordFiles) < 2 {
		discordFiles = regexp.MustCompile(`(?i)https?://[^\"'\s]+assets/[^\"'\s]+\.js`).FindAllString(string(responeBody), -1)
	}

	if len(discordFiles) < 2 {
		return 0, fmt.Errorf("failed to parse asset js files from discord.com/app (found %d entries)", len(discordFiles))
	}

	// normalize path (remove any leading slash) before joining with domain if needed
	chosen := discordFiles[len(discordFiles)-2]
	chosen = strings.TrimSpace(chosen)
	chosen = strings.TrimPrefix(chosen, "//")
	chosen = strings.TrimPrefix(chosen, "/")

	var file_with_build_num string
	if strings.HasPrefix(strings.ToLower(chosen), "http") {
		file_with_build_num = chosen
	} else {
		file_with_build_num = "https://discord.com/" + chosen
	}
	...
	numMatches := regexp.MustCompile(`"[0-9]{6}"`).FindAllString(string(responeBody), -1)
	if len(numMatches) == 0 {
		return 0, fmt.Errorf("failed to find 6-digit build number in %s", file_with_build_num)
	}

	client_build_number_str := strings.Replace(numMatches[0], "\"", "", -1)
